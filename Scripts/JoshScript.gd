...
# Josh only ever charges HORIZONTALLY. He locks onto the player's Y
# position the instant a charge begins, snaps to that height, then
# sweeps the whole width of the arena at that fixed height.
...
const TELEGRAPH_TINT := Color(1.7, 1.7, 1.35)
...
func _start_charging(target: Vector2, arrival_time: float) -> void:
	...
	var dist = abs(target.x - global_position.x)
	dist = max(dist, 40.0)
	charge_speed = dist / max(arrival_time, 0.05)
	charge_direction = sign(target.x - global_position.x)
...
func _on_body_hitbox_area_entered(area: Area2D) -> void:
	...
	if state == State.CHARGING and area.is_in_group("carter_body"):
...
func _play_recovering() -> void:
	if sprite:
		sprite.modulate = Color(1, 1, 1)
		# Only the charge art is side-facing; every other pose is drawn
		# front-on, so the mirror has to come off as soon as the dash ends.
		sprite.flip_h = false
	if animation_player:
