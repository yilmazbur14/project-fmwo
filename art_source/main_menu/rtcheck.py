import sys
from lib import read_png, validate_png
def norm(px):
...
alpha = set(p[3] for row in pb for p in row)
print('%s vs %s: %dx%d, %d differing pixels, alpha values in export %s' % (a.split('/')[-1], b.split('/')[-1], wa, ha, len(diff), sorted(alpha)))
for d in diff[:10]:
    print('  ', d)
validate_png(b)
