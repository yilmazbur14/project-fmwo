"""zoom.py in.png out.png scale [bg r,g,b] [fw] [fh] -> nearest-neighbour upscale; optional frame grid lines"""
import sys, os
...
fh = int(sys.argv[6]) if len(sys.argv) > 6 else 0
w, h, px = read_png(src)
out = []
for y in range(h):
    row = []
...
if fh:
    for x in range(w * s):
        for f in range(1, h // fh):
            out[f * fh * s][x] = (255, 255, 255, 255)
